...
import sys
import time
import serial      # [추가] 피코 시리얼 통신
import braillify   # [추가] 점자 변환 라이브러리
...
                
        print(f"[피코 전송] 단어: {text} -> 패턴: {bit_pattern}")
        ser.write((bit_pattern + "\n").encode())
    except Exception as e:
        print(f"[점자 전송 에러]: {e}")
def send_braille_to_pico(text):
    if ser is None:
...
                bit_pattern += col1 + col2
                
        # [핵심 수정] 데이터의 시작('<')과 끝('>') 마커를 씌워서 패킷화
        packet = f"<{bit_pattern}>"
        
        print(f"[피코 전송] 단어: {text} -> 패킷: {packet}")
        
        # 줄바꿈(\n)은 피코의 버퍼 비우기(flush) 용도로 끝에 하나 붙여줍니다.
        ser.write((packet + "\n").encode())
        
    except Exception as e:
        print(f"[점자 전송 에러]: {e}")
...
    send_braille_to_pico(target_word)  # [추가] 피코로 점자 데이터 전송
...
    # 1. [신규 추가] 랜덤 주제 선택 기능
    # 주제를 못 정했을 때 "랜덤 주제", "아무거나", "무작위" 등을 말하면 작동합니다.
...
    # 2. 특정 주제 직접 선택 (예: "과일", "동물" 등)
    for idx, topic in enumerate(topic_list):
        if topic in text:
            load_topic(idx)
            return True

    # 3. 처음 상태로 돌아가기 (원하는 주제 다시 묻기)
...
    # 4. 다른 주제로 랜덤 변경 (진행 중 다른 주제로 넘어가고 싶을 때)
